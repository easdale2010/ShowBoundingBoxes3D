...
namespace gamelib3d
{
    ...
    public class staticmesh
    {
        ...
        public staticmesh(ContentManager content, string modelname, float msize, Vector3 mpos, Vector3 mrot, Boolean mvis)
        {
    ...
    public class BoundingBoxes
    {
        ...
        public BoundingBoxes() { }         // Empty constructor which is only in to stop crashes 

        // Constructor which loads 3D model and sets it up with size, position and initial rotation
        public BoundingBoxes(Vector3 position, Vector3 boxsize)
        {
            // Create a bounding box around the 3D model
            Vector3 leftcorner = position;
            Vector3 rightcorner = position;
    ...
    public class model3d : staticmesh
    {
        ...
        // Constructor which loads 3D model and sets it up with size, position and initial rotation
        public model3d(ContentManager content, string modelname, float msize, Vector3 mpos, Vector3 mrot, float rspeed, float mpower, float mweight,Boolean mvis)
        {
            graphic = content.Load<Model>(modelname);                   // Load the 3D model from the ContentManager
            transforms = new Matrix[graphic.Bones.Count];               // make an array of transforms, one for each 'bone' in the 3d model
            graphic.CopyAbsoluteBoneTransformsTo(transforms);           // copy the transforms from the 3d model into this array ready for use
        ...
        public void moveAI(float gtime, int groundpos)
        {
            if (visible)
            {
                //                if (MathHelper.ToDegrees(rotation.Y) >= 360) rotation.Y = 0;
                ...
                if (MathHelper.ToDegrees(rotation.Y) > 360) rotation.Y -= MathHelper.ToRadians(360);
        ...
        public void automove1(int rightside, int leftside, int top, int bottom, int groundpos, Vector3 rot)
        {
            if (visible)
            {
                if (MathHelper.ToDegrees(rotation.Y) >= 360) rotation.Y = 0;
        ...
        public void jump(float forceappplied, SoundEffect jumpy)
        {
            if (visible && velocity.Y == 0)
            {
